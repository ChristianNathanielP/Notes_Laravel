<x-app-layout>
    <div class="note-container single-note">
        <h1 class="font-fredoka font-semibold text-2xl lg:text-3xl text-white mb-5">Edit your note</h1>
        <form action="{{ route('note.update', $note) }}" method="POST" class="note">
            @csrf
            @method('PUT')
            <textarea name="note" rows="10" class="w-full bg-biru2 bayangan2 font-pattrick_hand text-putih text-lg leading-loose p-5 active:ring-0" placeholder="Enter your note here">
                {{ old('note', $note->note) }}
            </textarea>
